...
def _try_claim(session, game_id: str, metric_key: str, worker_id: str) -> tuple[bool, str | None]:
    ...
    Returns (owned, existing_status):
      ...
      (False, 'in_progress')— another worker owns it (or crashed), skip
    """
    ...
    row = (
        session.query(MetricJobClaim.status)
        .filter(
    ...
    existing_status = row.status if row else _STATUS_DONE
...
def compute_game_metrics(self, game_id: str, metric_key: str) -> dict:
    ...
    try:
        results = _compute_with_deadlock_retry(SessionLocal, game_id, metric_key)
...
def _compute_with_deadlock_retry(SessionLocal, game_id: str, metric_key: str) -> list:
    for attempt in range(3):
        try:
            with SessionLocal() as session:
                return run_for_game_single_metric(session, game_id, metric_key, commit=True)
