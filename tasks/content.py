...
def _build_game_analysis_description(target_date: date, game_id: str) -> str:
    ...
    return (
        ...
        "You must materialize the image files yourself before calling Funba. Funba stores provided image assets; it does not search, generate, or capture them for you.\n"
        ...
        "Image metadata fields such as `type`, `query`, `target`, `prompt`, `player_id`, and `player_name` are provenance only.\n"
        "The only required storage field for each image asset is `file_path`, pointing at a local file already prepared by the agent.\n\n"
        "Per post, aim for a mix like:\n"
        "- 3-4 screenshots or page captures prepared by you (rankings, game pages, player pages, metric pages)\n"
        "- 3-4 real game/editorial photos collected by you\n"
        "- 2-3 stylized variants prepared by you when they add real value\n"
        "- Headshots are allowed, but do not rely on them as the main image set because they feel repetitive.\n\n"
        ...
        "```json\n"
        "\"images\": [\n"
        "  {\"slot\": \"img1\", \"type\": \"web_search\", \"file_path\": \"/tmp/maxey_game_photo.jpg\", \"query\": \"Tyrese Maxey driving to the basket vs Wizards April 2026\", \"note\": \"马克西比赛现场图\"},\n"
        "  {\"slot\": \"img2\", \"type\": \"screenshot\", \"file_path\": \"/tmp/scoring_consistency.png\", \"target\": \"https://funba.app/metrics/scoring_consistency?season=22025\", \"note\": \"得分稳定性排行\"},\n"
        "  {\"slot\": \"img3\", \"type\": \"ai_generated\", \"file_path\": \"/tmp/maxey_poster.png\", \"prompt\": \"Tyrese Maxey exploding through the lane, comic-book energy, dramatic arena lights, dynamic motion, editorial sports poster\", \"note\": \"马克西风格化赛场图\"}\n"
        "]\n"
        "```\n\n"
