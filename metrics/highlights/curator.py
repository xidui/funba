...
## 筛选原则
1. **硬优先级**：候选列表按指标窗口分层 — 生涯 > 近5场 > 近3场 > 当前赛季。
   `metric_key` 以 `_career` 结尾的是生涯 rank_crossing / approaching（历史级事件），
   以 `_last5` / `_last3` 结尾的是近期窗口，其余（无后缀）是当前赛季。
   **只要候选里有 career 级的 milestone rank_crossing，`hero` 必须从 career milestones 里挑**，
   不能让当前赛季的高分盖过历史里程碑（比如 "赛季单场 62 分" 不能压掉 "生涯季后赛助攻超越隆多"）。
2. 当前赛季（concrete-season）metrics 在赛季早期基本都是噪音（2-3 场值就都很小），能丢尽量丢；
   除非确实出色（例如 score > 40 且排名赛季 top 3），否则**最多作 notable，不能作 hero**。
3. 不重复：同一球员的同类指标（例如"得分最高"和"赛季得分最高"），只保留最能讲故事的一条。
4. **同一球员最多输出 2 条**（总共 hero + notable 合计）。但若一个球员同场触发了多条 career milestone
   crossing（比如 Harden 同场超过 Rondo 和 Chris Paul），**这种稀有历史事件是例外，可以破 2 条限制**。
5. **必须 spread 到不同的球员**，覆盖比赛里多方面的亮点（除非 rule 4 例外情况）。
6. 球员要有**具体的名字**出现在 narrative 里。
7. 真实稀有性：rank 1 但值（value）很小（比如得分、助攻 < 5）多半是并列第一，不是真正突出，丢掉。rank/total > 0.2 不够 hero。
8. `source=milestone` 的候选代表本场发生了排名跨越或逼近目标事件；叙事必须优先使用候选里的 milestone_context/passed/target/thresholds，不要再脑补外部事实。
9. milestone 的 `event_type=absolute_threshold` 代表本场跨越绝对里程碑（如万分、100 次三双）。narrative 必须引用 threshold_label_zh / threshold_label_en 和 count_reached_before_this_game。
10. milestone 的 `event_type=approaching_absolute` 代表逼近绝对里程碑。narrative 用 threshold_label + new_gap 表达，例如"距万分仅差 47 分"。
