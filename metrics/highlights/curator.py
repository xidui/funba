...
6. **避开稀疏池子**。每条候选带 `pool_size` 字段 = 这条 (metric_key, season) 在排行榜池里的总行数。`pool_size < 5` 的候选不要选 hero —— 下游海报画 leaderboard 时池子太稀，结果就是几行真数据 + 一堆空槽位。同主题如果有 `_career` 兄弟候选（pool_size 通常更大、覆盖整个赛季类型的历史），优先选 career 视角；没有合适替代时，可以做 notable 而非 hero。
...
- **赛季阶段 + 窗口用词必须跟候选的 `scope_reference_zh/en` 一致**：
  ...
  - `last3_playoffs` / `last5_playoffs` / `last10_playoffs`：写 "过去3/5/10届季后赛" / "past 3/5/10 playoff seasons"。
  - `last3_regular` / `last5_regular` / `last10_regular`：写 "过去3/5/10个常规赛季" / "past 3/5/10 regular seasons"。
- **不重复指标本身的定义**。例如指标叫 "N-0 得分潮"，写 "打出 17-0 得分潮" 就够，别加 "对手 0 分"。
- **narrative 必须忠实反映候选的 `metric_description`**。description 是 metric 的权威定义。
...
- 只使用候选数据里给出的信息。**严禁编造**以下这类候选里没给的表达：
  ...
  - "队史第 X"（除非候选就是队史级）
  - "近 X 场最高"（last3/last5/last10 都是赛季窗口，不是场数；应写 "过去 X 个常规赛季" 或 "过去 X 届季后赛"）
...
{
  "hero": [
    {"metric_key": "<key>", "entity_id": "<id>", "rank_window": "<window>", "narrative_zh": "<中文>", "narrative_en": "<English>"}
  ],
  "notable": [
    {"metric_key": "<key>", "entity_id": "<id>", "rank_window": "<window>", "narrative_zh": "<中文>", "narrative_en": "<English>"}
  ]
...
## rank_window 字段（必填，用来下游决定排行榜池子大小）
取以下五个值之一，必须与你 narrative 里实际用的框架措辞**严格一致**：
- `"season"` —— narrative 写 "本届季后赛 / 本赛季 / this postseason / this season" 时
- `"alltime"` —— narrative 写 "季后赛历史 / 常规赛历史 / NBA 历史 / playoff history / regular season history / all-time" 时
- `"last3"` / `"last5"` / `"last10"` —— narrative 写 "过去3/5/10届季后赛 / past 3/5/10 playoff seasons / past N regular seasons" 时

判断顺序：优先看候选的 `metric_window`（"career" → 用 `"alltime"`；"last3/5/10" → 同名；"season" → 通常 `"season"`）。
但 `metric_window="season"` 的候选有时排名亮点在历史维度（rank_snapshot.alltime 很靠前），如果你 narrative 写了 "playoff history / 季后赛历史"，那就把 rank_window 设为 `"alltime"`，二者必须配套。
"""
...
- `_last10` → **最近 10 个同类型赛季**（10 个 playoff 赛季，不是 10 场比赛）
- `_last5` → **最近 5 个同类型赛季**（5 个 playoff 赛季，不是 5 场比赛）
- `_last3` → **最近 3 个同类型赛季**
- `_per_game` → **单场粒度的排行榜**：每条候选指向一场具体比赛，pool 把所有(球员, 场)
  作为独立条目对比；`season_rank` 是该 (球员, 场) 在赛季所有进入榜单的单场表现里的位置，
  `alltime_rank` 是同 season-type 全史排名（季后赛 → 季后赛史所有同类单场榜单）。
  叙事写"赛季第 N / 季后赛史第 N（单场）"。**不要把它跟 base sibling 混淆**：
  base 是"该球员个人本赛季峰值",`_per_game` 是"该单场表现在所有同类单场里的位置"。
...
### 同球员同 metric 同时出现 base + `_per_game` 两条候选时，**只能保留一条**
两者 value 通常一致(球员季峰值场就是球员单场榜单上他的最佳条目),为避免重复:
- 如果 `_per_game` 的 `alltime_rank` 在前列(比如前 50)→ 选 `_per_game`,叙事写"季后赛史单场第 N"
- 否则 → 选 base sibling(更熟悉的"本届季后赛单场第 1")
- **绝对不要两张都写**(同一事实,同一球员,同一比赛,只换了 pool 视角)

### 关键字段
- `metric_description` / `metric_description_zh`：base metric 的描述。
  Sibling（_career/_last10/_last5/_last3）**复用 base 的描述文本** — 描述里如果写"单赛季累计"，
  sibling 要读成"在对应范围（career 所有赛季 / 最近 10 赛季 / 最近 5 赛季 / 最近 3 赛季）内累计"。
  **不要被描述里的"单赛季"字样误导** — 范围看 `metric_window` + `season`。
- `metric_window`：显式窗口标签 — `career` / `last10` / `last5` / `last3` / `season`
- `season`：具体 season token，例 `42025` / `all_playoffs` / `last10_playoffs`
- `scope_reference_zh/en`：该候选排名池的正确自然语言说法；写排名时优先照这个字段。
  ...
  - `last10_playoffs` + last10 → "过去10届季后赛" / "past 10 playoff seasons"
  - `last5_playoffs` + last5 → "过去5届季后赛" / "past 5 playoff seasons"
  - regular season 对应写 "本赛季" / "this season"、"常规赛历史" / "regular-season history"、"过去10/5个常规赛季" / "past 10/5 regular seasons"
...
- **近 10 / 近 5 / 近 3 个同类型赛季窗口** (`_last10` / `_last5` / `_last3`)：
  - 这里的 last10/last5/last3 是最近 10/5/3 个同类型赛季，不是最近 10/5/3 场比赛。
  - 作 notable，按 `scope_reference_zh/en` 写 "过去10届季后赛" / "past 10 playoff seasons" 等等。
...
- **避开稀疏池子**：每条候选带 `pool_size` 字段 = 这条 (metric_key, season) 在排行榜池里的总行数。`pool_size < 5` 的候选不要做 hero —— 下游海报画 leaderboard 时池子太稀，会留下一堆空槽位。典型场景是"门槛型"指标（10+ blocks、20+ rebounds 一场这种），本赛季可能就 1 个人达成。这种情况下：如果同主题的 `_career` 兄弟候选在池里且 pool_size 更大，**优先选 career 视角**（叙事框架改成 "playoff history / 季后赛历史"）；没有合适替代就做 notable 或丢掉，不要硬上 hero。
...
- **赛季阶段 + 窗口用词必须跟候选 `scope_reference_zh/en` 一致**：playoff 的 season/career/last3/last5/last10 分别写 "本届季后赛 / playoff history / 过去3届季后赛 / 过去5届季后赛 / 过去10届季后赛"，英文分别写 "this postseason / playoff history / past 3 playoff seasons / past 5 playoff seasons / past 10 playoff seasons"；regular 才写 "this season" 或 "regular-season history"。
...
{
  "hero": [
    {"metric_key": "<key>", "entity_id": "<player_id>", "rank_window": "<window>", "narrative_zh": "...", "narrative_en": "..."}
  ],
  "notable": [
    {"metric_key": "<key>", "entity_id": "<player_id>", "rank_window": "<window>", "narrative_zh": "...", "narrative_en": "..."}
  ]
...
## rank_window 字段（必填）
取以下五个值之一，必须与你 narrative 里实际用的框架措辞**严格一致**：
- `"season"` —— "本届季后赛 / 本赛季 / this postseason / this season"
- `"alltime"` —— "季后赛历史 / 常规赛历史 / NBA 历史 / playoff history / regular season history / all-time"
- `"last3"` / `"last5"` / `"last10"` —— "过去3/5/10届季后赛 / past 3/5/10 playoff seasons / past N regular seasons"

通常 `metric_window` 是什么就填什么（career → `"alltime"`，last3/5/10 → 同名，season → `"season"`）。
但 `metric_window="season"` 的候选若 alltime 排名亮眼、你叙事写了 "playoff history"，就要填 `"alltime"`——必须与 narrative 配套。
"""
...
8. **避开稀疏池子**。每条候选带 `pool_size` 字段 = 这条 (metric_key, season) 在排行榜池里的总行数。`pool_size < 5` 的候选不要选 hero —— 下游海报画 leaderboard 时池子太稀，会留下一堆空槽位。同主题如果有 `_career` 兄弟候选（pool_size 通常更大、覆盖整个赛季类型的历史），优先选 career 视角；没有合适替代就做 notable 或丢掉。
...
- **赛季阶段 + 窗口用词必须跟候选 `scope_reference_zh/en` 一致**：playoff 的 season/career/last3/last5/last10 分别写 "本届季后赛 / 季后赛历史 / 过去3届季后赛 / 过去5届季后赛 / 过去10届季后赛"，英文分别写 "this postseason / playoff history / past 3 playoff seasons / past 5 playoff seasons / past 10 playoff seasons"；regular 才写 "this season" 或 "regular-season history"。
...
{
  "hero": [
    {"metric_key": "<key>", "entity_id": "<team_id>", "rank_window": "<window>", "narrative_zh": "...", "narrative_en": "..."}
  ],
  "notable": [
    {"metric_key": "<key>", "entity_id": "<team_id>", "rank_window": "<window>", "narrative_zh": "...", "narrative_en": "..."}
  ]
...
_VALID_RANK_WINDOWS = ("season", "alltime", "last3", "last5", "last10")


def _coerce_rank_window(value: Any, *, scope_window: str | None = None) -> str | None:
    """Validate the LLM's `rank_window` field. Falls back to the candidate's
    `metric_window` when the LLM omitted or garbled the field — that catches
    most cases since `metric_window` is deterministic for sibling variants.
    Returns None when nothing usable is available so the snapshot stays
    explicit about not-knowing rather than silently defaulting to "season"."""
    raw = str(value or "").strip().lower()
    if raw in _VALID_RANK_WINDOWS:
        return raw
    fallback = str(scope_window or "").strip().lower()
    if fallback == "career":
        return "alltime"
    if fallback in _VALID_RANK_WINDOWS:
        return fallback
    return None


def _snapshot_entry(entry: dict, narrative_zh: str, narrative_en: str, rank_window: str | None) -> dict:
    ...
    def _take(section: str, limit: int) -> list[dict]:
        ...
        for pick in (parsed.get(section) or [])[:limit]:
            ...
            if raw is None:
                ...
                continue
            rank_window = _coerce_rank_window(
                pick.get("rank_window"), scope_window=raw.get("scope_window") or raw.get("metric_window")
            )
            out.append(_snapshot_entry(raw, narrative_zh, narrative_en, rank_window))
        return out
...
def _snapshot_triggered_entry(card: dict, narrative_zh: str, narrative_en: str, rank_window: str | None) -> dict:
    snap = {
    ...
    *,
    ...
    def _take(section: str, limit: int) -> list[dict]:
        ...
        for pick in (parsed.get(section) or [])[:limit]:
            ...
            out.append(_snapshot_triggered_entry(raw, narrative_zh, narrative_en, rank_window))
        return out
