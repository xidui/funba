...
## 系统说明（sibling / season 生成规则）
每条候选有以下关键字段，解读前先看清楚：
- `metric_key`：例 `season_total_assists` / `season_total_assists_career` / `_last5` / `_last3`。
  后缀决定**统计窗口**：
  - 无后缀 → 当前 concrete season（如 `42025` = 2025-26 playoffs）
  - `_career` → 该类型全部比赛累计（all_regular / all_playoffs / all_playin）
  - `_last5` → 本球员最近 5 场该类型比赛
  - `_last3` → 最近 3 场
- `metric_description` / `metric_description_zh`：base metric 的描述。sibling（_career/_last5/_last3）
  复用 base 的描述文本 — 但实际窗口看 `metric_window` 和 `season` 字段，**别被"单赛季"字样误导**。
- `metric_window`：显式窗口标签 — `career` / `last5` / `last3` / `season`
- `season`：具体 season，`42025` 是 2025-26 季后赛，`all_playoffs` 是生涯季后赛累计，etc.
- `season_rank` / `alltime_rank`：该球员在这个窗口 pool 里的排名。
  - 对 `_career` variants：season=all_playoffs，`season_rank` 就是历史全时排名
  - 对 base：season=42025，`season_rank` 是当前赛季排名
- `milestone_context.game_delta`：**本场**实际贡献（单场数值）

## 🚨 value 字段的含义（**最常见的错读 — 读错会造假**）
- `season_total_*` / `_career` / `_last5` / `_last3` 这些**累加类** metric：
  - `value` / `value_num` 是 **"截至本场打完的累计总值"**，**不是本场数值**。
  - 例 1：`season_total_points` value=62 season=42025 = "本赛季（到目前为止）累计 62 分"，**不是**"单场 62 分"。
  - 例 2：`season_total_assists_career` value=1139 season=all_playoffs = "生涯季后赛累计 1139 助攻"。
  - 本场的实际贡献在 `milestone_context.game_delta`。正确叙事：
    * ✅ "米切尔本场砍 30 分，季后赛累计 62 分升至 #1"
    * ✅ "哈登本场 4 助攻，生涯季后赛累计 1139 超越隆多"
    * ❌ "米切尔砍 62 分" / "单场 62 分"（完全错，accumulator 不是单场）
- 真正的"单场"指标：
  - `best_single_game_*` (runlog) = 球员单场最高值
  - `highest_*_in_a_game` / `single_game_*` = 单场极值
  - 看 metric_name：带 "Best Single-Game" / "Single Quarter" / "In A Game" 的才是单场
- 带 "Season Total" / "Career" / "Total" / "_career" 的就是累计，写的时候加"累计"、"合计"、"达到 X"、"来到 X"字眼
...
- **近 5 场 / 近 3 场 milestone** (`_last5` / `_last3`)：
  - 作 notable，讲"最近几场连续 X" 这种叙事
...
def _window_label(metric_key: str | None, season: str | None) -> str:
    ...
    if str(season or "").startswith("all_"):
...
def _build_triggered_llm_input(kind: str, cards: list[dict]) -> list[dict]:
    ...
    for c in cards:
        entry = {
            ...
            "metric_window": c.get("metric_window"),
            "season": c.get("season"),
            "source": c.get("source"),
