...
def _build_game_posters(session, game, *, lang: str) -> list[dict]:
    """Hero card posters generated by the curator pipeline, enriched for the
    game page strip. Returns [] when nothing was generated for this game.
    """
    from social_media.hero_poster import (
        ...
        list_hero_posters_for_game,
    )
    from db.models import MetricDefinition, Player, Team

    raw = list_hero_posters_for_game(game.game_id)
    if not raw:
        return []

    is_zh = lang == "zh"
    base_dir = _media_root() / HERO_POSTERS_SUBDIR / _safe_segment(str(game.game_id))

    metric_keys: set[str] = set()
    for entry in raw:
        if entry["metric_key"]:
            metric_keys.add(entry["metric_key"])
            metric_keys.add(_strip_variant_suffix(entry["metric_key"]))
    ...
    if metric_keys:
        for row in (
            ...
            metric_labels[row.key] = (row.name or "", row.name_zh or "")

    player_ids = {e["entity_id"] for e in raw if e["scope"] == "player" and e["entity_id"]}
    team_ids = {e["entity_id"] for e in raw if e["scope"] == "team" and e["entity_id"]}
    ...
    if player_ids:
        for row in (
            ...
            players[row.player_id] = (row.full_name or "", row.full_name_zh or "")
    if team_ids:
        for row in (
            session.query(Team.team_id, Team.full_name, Team.full_name_zh)
            .filter(Team.team_id.in_(team_ids))
            .all()
        ):
            teams[row.team_id] = (row.full_name or "", row.full_name_zh or "")
    ...
    for entry in raw:
        ...
        thumb_exists = (base_dir / thumb_name).exists()
        thumb_url = f"/media/hero_posters/{game.game_id}/{thumb_name}" if thumb_exists else f"/media/hero_posters/{game.game_id}/{filename}"
        full_url = f"/media/hero_posters/{game.game_id}/{filename}"

        metric_key = entry["metric_key"]
        labels = metric_labels.get(metric_key) or metric_labels.get(_strip_variant_suffix(metric_key)) or ("", "")
        name_en, name_zh = labels
        caption = (name_zh if is_zh else name_en) or name_en or name_zh or metric_key.replace("_", " ").title()

        sub_caption = ""
        if entry["scope"] == "player":
            pl = players.get(entry["entity_id"])
            if pl:
                sub_caption = (pl[1] if is_zh else pl[0]) or pl[0] or pl[1] or ""
        elif entry["scope"] == "team":
            tm = teams.get(entry["entity_id"])
            if tm:
                sub_caption = (tm[1] if is_zh else tm[0]) or tm[0] or tm[1] or ""

        accent_color = "#1d3557"
        try:
            color_text = (base_dir / (stem + ".color.txt")).read_text(encoding="utf-8").strip()
            if color_text.startswith("#") and len(color_text) in (4, 7, 9):
                accent_color = color_text
...
def register_detail_routes(
    ...
    *,
        ...
        with SessionLocal() as session:
            ...
            game_posters = (
                _build_game_posters(
                    session,
                    game,
                    lang=("zh" if request.path.startswith("/cn/") else "en"),
                )
                if game_status == GAME_STATUS_COMPLETED
                else []
            )
    ...
    def serve_hero_poster(game_id: str, filename: str):
        from social_media.hero_poster import _media_root, _safe_segment, HERO_POSTERS_SUBDIR

        media_dir = (_media_root() / HERO_POSTERS_SUBDIR / _safe_segment(str(game_id))).resolve()
        ...
        return make_response(
            ...
            200,
            {"Content-Type": mime_type, "Cache-Control": "public, max-age=86400, immutable"},
        )

    app.add_url_rule(
        "/media/hero_posters/<game_id>/<filename>",
        endpoint="serve_hero_poster",
        view_func=serve_hero_poster,
    )
