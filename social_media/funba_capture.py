...
def _player_metrics_plan(*, cards: int = 4) -> dict[str, object]:
    ...
    return {
        "selectors": [
            ...
            ".metrics-section .metrics-sub-title",
            ".metrics-section .metrics-grid",
            f".metrics-section .metrics-grid .metric-card:nth-child({card_index})",
        ],
        "pad_x": 12,
        "pad_top": 12,
        "pad_bottom": 18,
...
def _player_metrics_adjustments(*, scope: str, cards: int = 6) -> dict[str, object]:
    if scope == "career":
        return {
            "remove_selectors": [
                ".metrics-section .metrics-sub-title:first-of-type",
                ".metrics-section .metrics-grid:first-of-type",
            ],
            "limit_grid_cards": {
                ".metrics-section .metrics-grid:first-of-type": max(1, int(cards)),
            },
        }
    return {
        "remove_selectors": [
            ".metrics-section .metrics-sub-title:nth-of-type(n+2)",
            ".metrics-section .metrics-grid:nth-of-type(n+2)",
        ],
        "limit_grid_cards": {
            ".metrics-section .metrics-grid:first-of-type": max(1, int(cards)),
        },
...
def capture_funba_url(url: str, output_path: str, *, wait_ms: int = 4000) -> None:
...
def cmd_capture(args: argparse.Namespace) -> None:
    ...
    if command == "url":
        capture_funba_url((args.url or "").strip(), output, wait_ms=wait_ms)
